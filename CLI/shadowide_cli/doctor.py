"""
Health checks for ShadowIDE CLI environments.
"""
...
def _check_python_version() -> DoctorCheck:
    ...
    return DoctorCheck("Python", "fail", f"Python {version_text} is too old; ShadowIDE CLI requires 3.8+.")
...
def _check_config_file(config: Config, config_path: Path) -> DoctorCheck:
    ...
    if not config_path.exists():
        return DoctorCheck(
            "Configuration",
            "warn",
            f"No config file found at {config_path}. Run `shadowide --init` to create one.",
        )
    ...
    if not config.is_initialized():
        return DoctorCheck(
            "Configuration",
            "warn",
            f"Config file exists at {config_path}, but setup is incomplete. Run `shadowide --init`.",
        )
...
def _check_workspace(cwd: Path) -> DoctorCheck:
    ...
    return DoctorCheck(
        "Workspace",
        "warn",
        f"No git repository detected from {cwd}. ShadowIDE works best inside a project checkout.",
    )
