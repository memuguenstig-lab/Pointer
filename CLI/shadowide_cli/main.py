#!/usr/bin/env python3
"""
Main entry point for ShadowIDE CLI.
"""
...
app = typer.Typer(
    name="shadowide",
    help="ShadowIDE CLI - AI-powered local codebase assistant",
    no_args_is_help=False,
    invoke_without_command=True,
)
config_app = typer.Typer(help="Inspect and update ShadowIDE CLI configuration.")
...
def main() -> None:
    """Entry point for the shadowide command."""
    app()
...
def cli_main(
    ...
    if version:
        ...
        console.print(f"ShadowIDE CLI v{__version__}")
        return
...
def _initialize_config(config: Config, config_path: Optional[str] = None) -> bool:
    ...
    console.print(
        Panel.fit(
            "[bold blue]Welcome to ShadowIDE CLI![/bold blue]\n\n"
            "This is your first time running ShadowIDE CLI. Let's set up your configuration.",
            title="Initialization",
        )
    )

    response = typer.confirm("Initialize ShadowIDE CLI?", default=True)
    if not response:
        return False
...
def init_command(
    ...
    """Initialize ShadowIDE CLI configuration."""
    ensure_config_dir()
    config = Config.load(config_path)
...
def _build_status_table(config: Config, config_path: Optional[str] = None) -> Table:
    ...
    table = Table(title="ShadowIDE CLI Status")
...
def context_show_command(
    ...
    table = Table(title="ShadowIDE CLI Context")
    table.add_column("Field", style="bold")
    table.add_column("Value", overflow="fold")
...
def context_files_command(
    ...
    table = Table(title="ShadowIDE CLI Context Files")
    table.add_column("File", style="bold")
...
def context_stats_command(
    ...
    table = Table(title="ShadowIDE CLI Context Stats")
...
def chats_list_command(
    ...
    table = Table(title="ShadowIDE CLI Chats")
...
def chats_current_command(
    ...
    table = Table(title="ShadowIDE CLI Current Chat")
    table.add_column("Field", style="bold")
    table.add_column("Value", overflow="fold")
...
def models_command(
    ...
    table = Table(title="ShadowIDE CLI Models")
...
def doctor_command(
    ...
    """Run basic health checks for the local ShadowIDE CLI setup."""
    ensure_config_dir()
    config = Config.load(config_path)
    applied_fixes = []
    if fix:
    ...
    table = Table(title="ShadowIDE CLI Doctor")
